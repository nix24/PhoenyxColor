...
>
	...
	{#if showInteractiveHandles && gradient}
		...
		{#if gradient.type === "linear" && angleHandlePosition}
			<!-- Angle line -->
			<svg class="absolute inset-0 pointer-events-none" style:width="100%" style:height="100%">
				<line
					x1={actualSize.width / 2}
					y1={actualSize.height / 2}
					x2={angleHandlePosition.x}
					y2={angleHandlePosition.y}
					stroke="white"
					stroke-width="2"
			...
			<div
				...
				style:left="{actualSize.width / 2 - 6}px"
				style:top="{actualSize.height / 2 - 6}px"
			></div>
			...
			<button
				...
				style:left="{angleHandlePosition.x - 10}px"
				style:top="{angleHandlePosition.y - 10}px"
		...
		{#if (gradient.type === "radial" || gradient.type === "conic") && centerHandlePosition}
			<button
				type="button"
				class={cn(
				...
				style:left="{centerHandlePosition.x - 12}px"
				style:top="{centerHandlePosition.y - 12}px"
	...
	{#if showInteractiveHandles && meshPoints.length > 0}
		{#each meshPoints as point}
			<button
				...
				style:left="{(point.x / 100) * actualSize.width - 12}px"
				style:top="{(point.y / 100) * actualSize.height - 12}px"
