	...
	<div class="flex flex-col h-full">
		<div class="p-4 border-b border-white/5 bg-black/20">
			<h3 class="font-semibold text-white flex items-center justify-between">
				<span>Gradients ({filteredGradients.length})</span>
			...
			{:else}
				{#each filteredGradients as gradient, i (gradient.id)}
					...
					>
						...
						<!-- Gradient Preview -->
						<div
							class="h-12 rounded-lg border border-white/10 shadow-inner transition-all duration-300 group-hover:shadow-lg"
				...
				{#if app.gradients.gradients.length === 0}
					<div class="text-center py-8 text-text-muted/50">
						<Icon icon="material-symbols:gradient" class="h-12 w-12 mx-auto mb-2 opacity-30" />
