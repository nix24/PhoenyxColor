...
function findPointAt(x: number, y: number): MeshPoint | undefined {
	return points.find((point) => {
		const px = (point.x / 100) * width;
		const py = (point.y / 100) * height;
		return Math.sqrt((x - px) ** 2 + (y - py) ** 2) < 15;
	});
}
...
function handleMouseMove(e: MouseEvent) {
	...
	if (isDragging && dragPointId && onPointMove) {
		...
		// Update hover state
		hoveredPointId = findPointAt(x, y)?.id ?? null;
	}
}
...
function handleContextMenu(e: MouseEvent) {
	...
	const { x, y } = getCanvasCoordinates(e);
	const point = findPointAt(x, y);
	if (point) onPointRemove(point.id);
}
