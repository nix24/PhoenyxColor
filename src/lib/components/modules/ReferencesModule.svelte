...
onMount(() => {
	let cancelled = false;
	void app.references.whenReady().finally(() => {
...
async function addFiles(files: File[]) {
	...
	for (const file of validFiles) {
		if (await processFile(file)) successCount += 1;
	}
	if (successCount > 0) toast.success(`Added ${successCount} reference image(s)`);
	if (successCount < validFiles.length) {
		toast.error(`${validFiles.length - successCount} image(s) could not be processed`);
	}
}
...
function handleContextMenu(e: MouseEvent, reference: ValidatedReferenceImage) {
	...
	contextMenuPos = {
		...
		y: Math.max(8, Math.min(e.clientY, window.innerHeight - 152)),
	};
}
...
async function handleRetryLoad() {
	isLoading = true;
	try {
		if (app.references.loadError) await app.references.reload();
		else await app.references.save();
	} catch (error) {
		console.error("Reference recovery failed:", error);
		app.references.saveError = "Changes still could not be saved on this device.";
	} finally {
		isLoading = false;
	}
}
...
>
		...
		<div class="mx-4 @md:mx-6 mt-2">
			<div class="p-3 rounded-xl bg-white/5 border border-white/5">
				<div class="flex items-center gap-2 text-xs text-text-muted mb-2">
					<Icon icon="material-symbols:cloud-upload" class="w-4 h-4 text-phoenix-primary" />
					Processing images
				</div>
	...
	<ImageLibrary
		...
		{selectedImageId}
		{isLoading}
		loadError={app.references.loadError ?? app.references.saveError}
		{isDragOver}
