<script lang="ts">
    ...
    function generateMonochromaticColors(
        ...
        for (let i = 0; i < count; i++) {
            ...
            const newL = Math.max(
                10,
                Math.min(90, hsl.l + (i - 1) * lightStep),
            );
            const newS = Math.max(
                10,
                Math.min(100, hsl.s + (i % 2 === 0 ? satStep : -satStep)),
            );
    ...
    function fillPaletteWithHarmony(
                ...
                for (let i = 0; i < remainingSlots; i++) {
                    ...
                    harmoniousColors.push(
                        hslToHex(
                            ...
                            Math.max(
                                10,
                                Math.min(100, hsl.s + saturationVariation),
                            ),
        ...
        const uniqueColors = harmoniousColors.filter(
            (color) =>
                ...
                !harmoniousColors
                    .slice(0, harmoniousColors.indexOf(color))
                    .includes(color),
        );
        ...
        toast.success(
            `Filled ${uniqueColors.length} slots with ${harmonyType} harmony!`,
        );
    }
    ...
    function exportPalette(
        ...
        switch (format) {
            ...
            case "ase":
                toast.info(
                    "Adobe Swatch Exchange (.ase) export is planned for a future version.",
                );
                return;
            default:
    ...
    function exportPaletteAsImage(
        ...
        const canvasWidth =
            totalColors * swatchSize + (totalColors + 1) * padding;
        ...
        if (format === "svg") {
            ...
            const a = document.createElement("a");
            a.href = url;
            a.download = `${palette.name.replace(/[^a-z0-9]/gi, "_").toLowerCase() || "palette"}.svg`;
            a.click();
            URL.revokeObjectURL(url);
        ...
        palette.colors.forEach((color, index) => {
            ...
            ctx.fillStyle =
                getComputedStyle(document.documentElement)
                    .getPropertyValue("--bc")
                    .trim() || "#000000"; // DaisyUI base-content
            ctx.font = "12px monospace";
            ctx.textAlign = "center";
            ctx.fillText(
                color,
                x + swatchSize / 2,
                padding + swatchSize + textHeight / 1.5,
            );
        });
        ...
        canvas.toBlob((blob) => {
            if (blob) {
                ...
                const a = document.createElement("a");
                a.href = url;
                a.download = `${palette.name.replace(/[^a-z0-9]/gi, "_").toLowerCase() || "palette"}.png`;
                a.click();
                URL.revokeObjectURL(url);
    ...
    async function importPaletteFile() {
        ...
        input.onchange = async (e) => {
            ...
            try {
                    ...
                    if (jsonData.colors && Array.isArray(jsonData.colors)) {
                        importedPalette = {
                            ...
                            maxSlots:
                                jsonData.maxSlots ||
                                Math.max(
                                    appStore.state.settings.defaultPaletteSlots,
                                    jsonData.colors.length,
                                ),
                            tags: jsonData.tags || [],
                        };
                    }
                ...
                if (
                    importedPalette &&
                    importedPalette.colors &&
                    importedPalette.colors.length > 0
                ) {
                    // Ensure unique name
                    let finalName = importedPalette.name || "Imported Palette";
                    let counter = 1;
                    while (
                        appStore.palettes.some((p) => p.name === finalName)
                    ) {
                    ...
                    appStore.addPalette(
                        importedPalette as Omit<
                            ColorPalette,
                            "id" | "createdAt"
                        >,
                    );
                    toast.success(
                        `Palette "${finalName}" imported successfully!`,
                    );
                } else {
                    toast.error(
                        "Could not parse palette from file or file was empty/invalid.",
                    );
                }
            } catch (error: any) {
    ...
    async function createPaletteFromReference() {
        ...
        const reference = appStore.references.find(
            (r) => r.id === extractingFrom,
        );
        if (!reference) {
            toast.error("Reference not found");
            return;
        ...
        try {
            ...
            // Create new palette with extracted colors
            const paletteName = `${reference.name.split(".")[0]} Colors`;
            let uniqueName = paletteName;
            let counter = 1;

            while (appStore.palettes.some((p) => p.name === uniqueName)) {
                uniqueName = `${paletteName} ${counter}`;
                counter++;
            }
            ...
            extractingFrom = null;
            toast.success(
                `Created palette "${uniqueName}" with ${colors.length} colors!`,
            );
        } catch (error) {
...
<div class="h-full flex flex-col">
    ...
    <div class="flex items-center justify-between p-6 border-b border-base-300">
        <div>
            <h2 class="text-2xl font-bold text-base-content">Color Palettes</h2>
            <p class="text-sm text-base-content/70">
                Create and manage your color palettes
            </p>
        </div>
                    ...
                    >
                        {#each appStore.references as reference (reference.id)}
                            <li>
                                <button
                                    onclick={() =>
                                        startColorExtraction(reference.id)}
                                    type="button"
                        ...
                        >
                            ...
                            <div class="flex items-center justify-between mb-3">
                                <div class="flex-1 min-w-0">
                                    ...
                                    <p class="text-xs text-base-content/60">
                                        {palette.colors
                                            .length}/{palette.maxSlots} colors
                                    </p>
                                </div>
                            ...
                            <div class="grid grid-cols-6 gap-1">
                                ...
                                {/each}
                                <!-- Empty slots -->
                                {#each Array(palette.maxSlots - palette.colors.length) as _}
                                    <button
                ...
                <div class="p-6">
                    <div class="flex items-center justify-between mb-6">
                        <div>
                            ...
                            <p class="text-sm text-base-content/70">
                                {appStore.activePalette.colors.length} of {appStore
                                    .activePalette.maxSlots} colors
                            </p>
                        </div>
                            ...
                            <div class="dropdown dropdown-end">
                                ...
                                >
                                    <li>
                                        <button
                                            onclick={() =>
                                                fillPaletteWithHarmony(
                                                    appStore.activePalette!.id,
                                                    "complementary",
                                                )}
                                            type="button"
                                        >
            ...
            {:else}
                <div class="flex items-center justify-center h-full">
                    <div class="text-center text-base-content/70">
                        ...
                        <p class="mb-4">
                            Create or select a palette to start working
                        </p>
                        <button
...
{#if showColorPicker}
    <div
        class="fixed top-16 right-6 z-40 scale-in"
    ...
    >
        ...
        />
        <button
            class="btn btn-sm btn-circle btn-ghost absolute top-2 right-2 z-50"
            onclick={() => {
                if (
                    editingColorContext &&
                    selectedColor !== editingColorContext.originalColor
                ) {
                    const { paletteId, colorIndex } = editingColorContext;
                    const palette = appStore.palettes.find(
                        (p) => p.id === paletteId,
                    );
        ...
        <div class="bg-base-100 rounded-lg shadow-xl p-6 w-96 max-w-90vw">
            ...
            {#if extractingFrom}
                ...
                {#if reference}
                    <div class="mb-4">
                        <div
                            class="flex items-center space-x-3 p-3 bg-base-200 rounded-lg"
                        >
                            <div
