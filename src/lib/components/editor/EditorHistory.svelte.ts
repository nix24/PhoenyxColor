...
import type { CurveChannels, ImageLayer } from "$lib/types/image-editor";
...
export interface ImageEditorState {
	...
	// Drawing overlay in source-image coordinates
...
function mergeEditorState(
	...
	if (newState.appliedEffects) mergedState.appliedEffects = deepClone(newState.appliedEffects);
	if (newState.drawStrokes) mergedState.drawStrokes = deepClone(newState.drawStrokes);
	if (newState.cropRect) mergedState.cropRect = deepClone(newState.cropRect);
	if (newState.layers) mergedState.layers = deepClone(newState.layers);
	return mergedState;
}
...
export class EditorHistoryService {
	...
	pushState(newState: Partial<ImageEditorState>) {
		...
		this.currentState = mergeEditorState(this.currentState, newState);
	}
