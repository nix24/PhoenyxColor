...
> [!WARNING]
> The application now uses `src/lib/stores/root.svelte.ts` and the modular stores. Keep new state in those capability-specific stores rather than introducing a monolithic application store.
